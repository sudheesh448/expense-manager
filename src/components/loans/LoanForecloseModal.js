...
export default function LoanForecloseModal({ visible, item, accounts, activeUser, onClose, onSuccess }) {
  ...
  const handleForeclose = async () => {
    if (!bankId) {
      Alert.alert('Selection Required', 'Please select a bank account for the payment.');
      return;
    }
    ...
    Alert.alert(
      'Confirm Foreclosure',
      'This will close the loan permanently and delete all future scheduled payments. Continue?',
      [
        ...
        { 
          ...
          onPress: async () => {
            ...
            try {
              ...
              console.error(e);
              Alert.alert('Error', 'Failed to foreclose loan. Please check your balance.');
            } finally {
              setLoading(false);
            }
          }
      ...
      <View style={styles.overlay}>
        <View style={[styles.modalContent, { backgroundColor: theme.surface }]}>
          <View style={styles.header}>
            <View>
              <Text style={[styles.modalTitle, { fontSize: fs(18), color: theme.text }]}>Foreclose Loan</Text>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={[styles.infoBox, { backgroundColor: theme.primary + '08', borderColor: theme.primary + '20' }]}>
               <View style={styles.infoRow}>
                  <Text style={{ color: theme.textSubtle, fontSize: fs(12) }}>Outstanding Total</Text>
               ...
               <View style={[styles.infoRow, { marginTop: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: theme.border + '11' }]}>
                  <Text style={{ color: theme.textSubtle, fontSize: fs(11) }}>Actual Principal</Text>
                  <Text style={{ color: theme.text, fontSize: fs(11), fontWeight: '600' }}>{getCurrencySymbol(activeUser?.currency)}{(stats?.principal_original || 0).toLocaleString()}</Text>
               </View>
            </View>
            ...
            <View style={[styles.warningBox, { backgroundColor: '#fff7ed', borderColor: '#fed7aa' }]}>
              ...
              <Text style={{ color: '#9a3412', fontSize: fs(12), flex: 1, marginLeft: 8 }}>
                Foreclosing will stop all future monthly installments and settle the loan balance immediately.
              </Text>
            </View>

            <View style={styles.inputSection}>
              <Text style={[styles.label, { color: theme.text, fontSize: fs(13) }]}>Settlement Amount ({getCurrencySymbol(activeUser?.currency)})</Text>
              <TextInput
            ...
            <Text style={[styles.label, { color: theme.text, fontSize: fs(13), marginBottom: 12 }]}>Pay From Bank Account</Text>
...
const styles = StyleSheet.create({
  ...
  closeBtn: { padding: 4 },
  infoBox: { padding: 16, borderRadius: 16, borderWidth: 1, gap: 10, marginBottom: 16 },
  infoRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  warningBox: { flexDirection: 'row', padding: 12, borderRadius: 12, borderWidth: 1, marginBottom: 20, alignItems: 'center' },
