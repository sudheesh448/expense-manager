...
export const saveLendedInfo = async (activeUserId, data, currencySymbol) => {
  ...
  if (data.bankAccountId) {
    ...
    await database.runAsync(
      ...
      [txId, userId, 'EXPENSE', data.principal, data.loanStartDate, data.bankAccountId, `Lended: ${data.name}`, id, category.id]
    );
    await updateAccountBalanceSQL(database, data.bankAccountId, data.principal, 'EXPENSE', false);
  }
